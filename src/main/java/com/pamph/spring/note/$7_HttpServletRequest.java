...
public class $7_HttpServletRequest {
  ...
  *
  * 그래서 이때, Spring Web 에서 지원하는 Servlet 을 사용함으로서 우리는 HTTP 요청에 담긴 정보를 쉽게 꺼내서 사용할 수 있다.
  ...
  *
  * ▶ HTTP Request Message 의 startLine 정보 얻기
  ...
  *   - getQueryString(): HTPP 요청 URL 중 쿼리 문자열 부분을 반환. 쿼리 문자열이 없다면 null 반환.
  *   - isSecure(): HTTP 요청이 SSL을 통해 보안 연결로 전송되었는지 여부를 나타낸다. https 이면 true 아니면 false 반환
  *
  * ▶ HTTP Request Message 의 headerLine 정보 얻기
  ...
  *
  * ▶ HTTP Request Message 의 간단한 header 정보 얻기
  ...
  *
  *
  * ▶ HTTP Request Message 의 기타 정보 얻기
