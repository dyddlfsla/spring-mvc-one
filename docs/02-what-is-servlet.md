...
```
 ...
 개발자는 이 요청에 응답하기 위해 다음과 같은 내용들을 코드로 작성해야 한다.

1. 서버 TCP/IP 연결 대기, 소켓 연결
2. HTTP 요청 메시지를 parsing 해서 내용 읽기
3. POST 방식, /save URL 인지 확인
4. Content-Type 확인
5. HTTP 메세지의 본문 데이터 확인 (username, age 데이터 parsing)
6. 저장 프로세스 실행
7.  비즈니스 로직 실행(★ 의미있는 비즈니스 로직)
8. 데이터베이스에 저장 요청
9. HTTP 응답 메시지 생성 시작
10. 메시지 Header 작성
11. 응답 메시지 본문에 HTML 데이터 저장
12. TCP/IP 응답 전달, 소켓 종료  
``` html
          ↓
...
```
 ...
 사실 위 과정 중에서 가장 중요한 단계는 회원 정보를 직접 가공하는 비즈니스 로직 실행이다.  
...
```
서블릿은 HTTP 요청을 처리하고 응답을 생성하는 서버 측 컴포넌트이다.  
서블릿의 특징을 살펴보면 다음과 같다.

 - urlPattern 의 URL 이 호출되면 서블릿 코드가 실행된다.
 - HTTP 요청 정보를 담고 있는 HttpServletRequest 객체를 제공한다.
 - HTTP 응답 정보를 담고 있는 HttpServletResponse 객체를 제공한다.
...
```
  ...
  - 클라이언트로부터 HTTP 요청이 들어오면 WAS 는 Request, Response 객체를 새로 만들어서 서블릿 객체를 호출한다.
...
### ※ 서블릿 컨테이너
 WAS 내부의 서블릿 컨테이너가 서블릿을 관리하고, 요청마다 request/response 객체를 생성한다.
 즉, 서블릿 컨테이너는 서블릿 객체의 생성, 초기화, 호출, 소멸 등의 생명주기를 담당하는 컨테이너인 것이다.

 - 톰캣은 서블릿 컨테이너 기능을 포함한 WAS이다.
 - 서블릿 컨테이너는 서블릿 객체의 생성, 초기화, 호출, 종료 등의 생명주기를 관리한다.
 - 서블릿은 기본적으로 하나의 인스턴스만 생성되어 여러 요청에서 공유된다.
