 ...
 **정리**
 ...
 🚨 `공통 처리에 대한 필요성`  
 사실 가장 큰 단점은 공통 기능 처리에 대한 불편함이다.  
 다시 말해, HTTP 요청이 왔을때 가장 먼저 이 요청을 받아 선처리를 해줄 수 있는 존재가 필요한 것이다.  
 이때 프론트 컨트롤러 패턴(Front Controller)을 도입하면 이런 문제를 해결할 수 있을 것이다.  
 즉, 모든 요청이 도달하는 지점을 하나로 만들고 그 진입점에서 모든 요청에 필요한 공통 처리를 하는 것이다.
 스프링 MVC 의 핵심도 바로 프론트 컨트롤러에 있다.
