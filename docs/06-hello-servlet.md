...
```java
 ...
 ```
 Servlet 클래스는 HttpServlet 클래스를 상속해야 한다.  
 ☑️ public class HelloServlet extends HttpServlet {}  
 ☑️ 클래스에 @WebServlet 애노테이션을 적용한다.
   - name: Servlet 의 이름
   - urlPatterns: Servlet 에 연결할 URL 경로

 브라우저에서 매핑된 URL 로 HTTP 요청이 오면 서블릿 컨테이너는 protected void service(...) 메서드를 실행한다.

☑️ HttpServletRequest request: HTTP 요청 정보를 담고 있는 객체이다.
   - getParameter(): 객체에서 요청 정보를 조회한다.

☑️ HttpServletResponse response: HTTP 응답 정보를 생성하는 객체이다.
